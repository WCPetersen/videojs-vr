...
// import projections
import ProjectionMap from './projections/index.js';
...
class VR extends Plugin {
  ...
  changeProjection_(projection) {
    ...
    if (this.scene && this.movieScreens) {
      this.scene.remove(...this.movieScreens);
      delete this.movieScreens;
    }
    if (projection === 'AUTO') {
    ...
    } else if (projection && ProjectionMap[projection]) {
      const ProjectionClass = ProjectionMap[projection];
      const videoProjection = new ProjectionClass(this.videoTexture, this.options_);
      this.movieScreens = videoProjection.screens;
      this.scene.add(...this.movieScreens);
    }
